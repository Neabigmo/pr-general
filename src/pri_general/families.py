...
def annotate_family_records(records: list[dict], work_root: str | Path) -> dict[str, int | bool]:
    """Apply existing local MMseqs2/RNAcentral assignments without guessing."""
    root = Path(work_root)
    maps = {
    ...
    return {
        ...
        "assignment_files_present": sum(bool(mapping) for mapping in maps.values()) + int(bool(rfam)),
    }
